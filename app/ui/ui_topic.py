...
class TopicWindow:
    def __init__(self):
        ...
        tk.Button(frame, text="Add Topic", command=self.add_topic).pack(side="left", padx=4)
        tk.Button(frame, text="Edit Topic", command=self.edit_topic).pack(side="left", padx=4)
        tk.Button(frame, text="Delete Topic", command=self.delete_topic).pack(side="left", padx=4)
    ...
    def load_data(self):
        ...
        for row in cur.fetchall():
            self.tree.insert("", "end", iid=row["topic_id"], values=(row["topic_id"], row["name"], row["description"] or ""))
        conn.close()
    ...
    def add_topic(self):
        ...
        if dlg.result:
            conn = get_connection(); cur = conn.cursor()
            cur.execute("INSERT INTO topic (name, description) VALUES (?, ?)", (dlg.result["name"], dlg.result["description"]))
    ...
    def edit_topic(self):
        ...
        if dlg.result:
            conn = get_connection(); cur = conn.cursor()
            cur.execute("UPDATE topic SET name=?, description=? WHERE topic_id=?", (dlg.result["name"], dlg.result["description"], tid))
            conn.commit(); conn.close()
            self.load_data()
    ...
    def delete_topic(self):
        ...
        if not sel:
            ...
            return
        if not messagebox.askyesno("Confirm", "Delete selected topic? This will set topic_id NULL in related rows (if FK ON DELETE SET NULL)."):
            return
        tid = int(sel[0])
        conn = get_connection(); cur = conn.cursor()
